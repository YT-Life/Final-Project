...
def index(request): 
    return render(request, 'ksm/index.html')
...
def img_upload(request):
    if request.method == 'POST':
        ...
        if cf:
            Images(pic=cf).save()
            return redirect('ksm:color_extract')
        else:
            return redirect('ksm:img_upload')

    return render(request, 'ksm/img_upload.html')
...
def color_extract(request):
    ...
    # 만약 객체 검출이 4개가 안된다면,
    while len(ob_color_list) < 20:
        ob_color_list.append("#ffffff00")

    request.session['bg_color_list'] = bg_color_list
    request.session['ob_color_list'] = ob_color_list

    return redirect('ksm:print_extract')
...
def print_extract(request):
    ...
    return render(request, 'ksm/color_extract.html', context)
...
def recommend(request):
    ...
    if color_list:
        ...
        return render(request, 'ksm/recommend.html', context)
